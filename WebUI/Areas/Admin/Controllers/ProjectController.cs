...
namespace WebUI.Areas.Admin.Controllers
{
    ...
    {
        public async Task<IActionResult> Index(string? search, string? status, string? state, string? address, string? highLight, string? type, int? perPage, int? currentPage, string? order, string? startDate, string? endDate)
        {
            ...
            var form = new SearchRequest
            {
                ...
                ValuesFilter1 = type?.Split(",").ToList() ?? ProjectTypeConstant.GetAllProperties(),
                ValuesFilter2 = state?.Split(",").ToList() ?? ProjectStateConstant.GetAllProperties(),
                ValueFilter1 = highLight,
                ValueFilter2 = address,
            };

            var result = await Mediator.Send(new GetProjectsQuery(form));
